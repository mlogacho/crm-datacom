...
class CustomAuthToken(ObtainAuthToken):
    def post(self, request, *args, **kwargs):
        ...
        if not totp.verify(totp_code, valid_window=1):
                        return Response({"error": "Codigo de autenticacion invalido."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if totp.verify(code, valid_window=1):
            ...
            return Response({
                ...
                'email': user.email
                        ...
                        return Response({"error": "Codigo de verificacion incorrecto."}, status=status.HTTP_400_BAD_REQUEST)
